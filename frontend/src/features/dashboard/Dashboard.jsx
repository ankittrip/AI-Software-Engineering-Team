...
import { ShieldAlert, ShieldCheck, Terminal, Zap, Activity, GitBranch, Loader2 } from 'lucide-react';
...
export const Dashboard = () => {
  ...
  const stats = [
    { label: 'Total Repos Scanned', value: dashboardStats.totalReposScanned, icon: Terminal, color: 'text-accent', bg: 'bg-accent/10 border-accent/20' },
    { label: 'Avg. Security Score', value: dashboardStats.avgSecurityScore, icon: ShieldCheck, color: 'text-emerald-500', bg: 'bg-emerald-500/10 border-emerald-500/20' },
    { label: 'Critical Issues Open', value: dashboardStats.criticalIssues, icon: ShieldAlert, color: 'text-red-500', bg: 'bg-red-500/10 border-red-500/20' },
    { label: 'AI Agents Online', value: dashboardStats.agentsOnline, icon: Zap, color: 'text-amber-500', bg: 'bg-amber-500/10 border-amber-500/20' },
  ];
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-surface border border-border p-8 rounded-2xl relative overflow-hidden">
        <div className="relative z-10">
          ...
          <p className="text-muted text-sm max-w-lg">
            Your AI agent swarm is fully operational. You have {dashboardStats.criticalIssues} critical vulnerabilities that require your attention today.
          </p>
        </div>
        <Link
          to="/scans/new"
